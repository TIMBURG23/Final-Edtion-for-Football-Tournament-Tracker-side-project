    ...
    if st.session_state.teams:
        ...
        for t in st.session_state.teams:
            ...
            cards_html += (
                ...
                f"<div style=\"font-size:2rem;\">{b}</div>"
                f"<h3 style=\"font-size:1rem;\">{t}</h3>"
                f"<small style=\"font-family:'JetBrains Mono', monospace; color:var(--db-text-dim);\">PIN {masked_pin}</small>"
                "</div>"
            )
        cards_html += "</div>"
